...
Before training, we must convert text to numbers and scale our data!

| Column | Strategy |
|:---|:---|
| `Brand`, `Model` | Target Encoding (high cardinality) |
| `Gearbox`, `Fuel`, `Origin` | One-Hot Encoding |
| `Location`, `Sector` | Drop if too noisy (we omit them) |
| `FO` (First Owner) | Binary encode (Yes/No → 1/0) |
| `Condition` | Ordinal encoding |

```{python}
#| echo: true
# Example: Encoding First Owner & Condition
...
## Handling Mileage and Fiscal Power

Mileage is often a range string. We convert it to a mean integer and drop extreme outliers.
...
We predict the **log-transformed** price using a `GradientBoostingRegressor`.
...
We use a `ColumnTransformer` to handle different types of data cleanly.
...
How well does our model perform? Let's check the test set metrics:

```{python}
#| echo: true
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score

# Predict and convert back to MAD
...
> **Result:** With an **R² of ~0.85**, our model performs quite well! The Mean Absolute Percentage Error (MAPE) suggests predictions are off by roughly 18% on average.
